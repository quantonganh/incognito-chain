...
func (proof PaymentProof) verifyNoPrivacy(pubKey privacy.PublicKey, fee uint64, db database.DatabaseInterface, shardID byte, tokenID *common.Hash, version int8) (bool, error) {
	if version == privacy.TxVersion1{
		...
		for i := 0; i < len(proof.inputCoins); i++ {
			...
			cmSND := new(privacy.Point).ScalarMult(privacy.PedCom.G[privacy.PedersenSndIndex], proof.inputCoins[i].CoinDetails.GetSNDerivatorRandom())
		...
		for i := 0; i < len(proof.outputCoins); i++ {
			...
			cmSND := new(privacy.Point).ScalarMult(privacy.PedCom.G[privacy.PedersenSndIndex], proof.outputCoins[i].CoinDetails.GetSNDerivatorRandom())
		...
		return false, privacy.NewPrivacyErr(privacy.InvalidVersionProofErr, nil)
	}
...
func (proof PaymentProof) Verify(hasPrivacy bool, pubKey privacy.PublicKey, fee uint64, db database.DatabaseInterface, shardID byte, tokenID *common.Hash, version int8) (bool, error) {
	// has no privacy
	if !hasPrivacy {
		return proof.verifyNoPrivacy(pubKey, fee, db, shardID, tokenID, version)
	}

	return proof.verifyHasPrivacy(pubKey, fee, db, shardID, tokenID, version)
}
