...
func TestCoinCommitAll(t *testing.T) {
	for i:= 0; i<1000; i++{
		...
		coin.snDerivatorRandom = RandomScalar()
		coin.randomness = RandomScalar()
		coin.value = new(big.Int).SetBytes(RandBytes(2)).Uint64()
		coin.serialNumber = new(Point).Derive(PedCom.G[0], new(Scalar).FromBytesS(privateKey), coin.snDerivatorRandom)
		coin.CommitAll()
		...
		cmTmp.Add(cmTmp, new(Point).ScalarMult(PedCom.G[PedersenSndIndex], coin.snDerivatorRandom))
...
func TestCoinMarshalJSON(t *testing.T) {
	...
	for i:= 0; i < 1000; i++ {
		...
		coin.serialNumber = PedCom.G[0].Derive(PedCom.G[0], new(Scalar).FromBytesS(privateKey), coin.snDerivatorRandom)
		coin.CommitAll()
		coin.info = []byte("Incognito chain")
...
func TestInputCoinBytesSetBytes(t *testing.T) {
	for i:= 0; i< 1000; i++ {
		...
		coin.CoinDetails.snDerivatorRandom = RandomScalar()
		coin.CoinDetails.randomness = RandomScalar()
		coin.CoinDetails.value = uint64(100)
		coin.CoinDetails.serialNumber = PedCom.G[0].Derive(PedCom.G[0], new(Scalar).FromBytesS(privateKey), coin.CoinDetails.snDerivatorRandom)
...
func TestOutputCoinBytesSetBytesWithMissingFields(t *testing.T) {
	...
	coin.CoinDetails.randomness = RandomScalar()
	coin.CoinDetails.value = uint64(100)
	//coin.CoinDetails.serialNumber = PedCom.G[0].Derive(PedCom.G[0], new(Scalar).FromBytes(SliceToArray(privateKey)), coin.CoinDetails.snDerivatorRandom)
	//coin.CoinDetails.CommitAll()
	coin.CoinDetails.info = []byte("Incognito chain")
	coin.Encrypt(paymentAddr.Tk)
