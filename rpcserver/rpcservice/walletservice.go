...
func (walletService WalletService) ListAccounts() (jsonresult.ListAccounts, *RPCError) {
	...
	for accountName, account := range accounts {
		...
		outCoins, _, err := walletService.BlockChain.GetListOutputCoinsByKeysetV2(&account.Key.KeySet, shardIDSender, prvCoinID, 0, 0)
		if err != nil {
...
func (walletService WalletService) GetBalanceByPrivateKey(privateKey string) (uint64, *RPCError) {
	...
	outcoints, _, err := walletService.BlockChain.GetListOutputCoinsByKeysetV2(keySet, shardIDSender, prvCoinID, 0, 0)
	log.Println(err)
	if err != nil {
